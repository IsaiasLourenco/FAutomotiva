...
HTML;
    ...
    for ($i = 0; $i < $linhas; $i++) {
        $id = $res[$i]['id'];
        $frequencia = htmlspecialchars($res[$i]['frequencia']);
        $dias = htmlspecialchars($res[$i]['dias']);
        
        echo <<<HTML
            <tr>
                <td><input type="checkbox" id="seletor-{$id}" class="form-check-input" onchange="selecionar('{$id}')">&nbsp;{$frequencia}</td>
                <td>{$dias}</td>
                <td>
                    <a href="#" onclick="editar('{$id}', '{$frequencia}', '{$dias}')" title="Editar">
...
<script>
    ...
    function editar(id, nome, taxa) {
        $('#mensagem').text('');
        $('#titulo_inserir').text('Editar Frequência');
        $('#id').val(id);
        $('#nome-frequencia').val(nome);
        $('#dias-frequencia').val(taxa);
