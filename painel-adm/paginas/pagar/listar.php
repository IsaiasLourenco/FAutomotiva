...
<style>
    .conta-paga { background-color: #e8f5e9 !important; border-left: 4px solid #4caf50 !important; }
    .conta-nao-paga { background-color: #ffebee !important; border-left: 4px solid #f44336 !important; }
    .conta-vencida { background-color: #ffcdd2 !important; border-left: 4px solid #d32f2f !important; font-weight: 600; }
    .conta-paga:hover, .conta-nao-paga:hover, .conta-vencida:hover { filter: brightness(0.95); }
</style>
...
<script>
    $(document).ready(function() {
        ...
        var table = $('#tabela').DataTable({
            ...
            "language": { "url": "https://cdn.datatables.net/plug-ins/1.13.4/i18n/pt-BR.json", "emptyTable": "Nenhum Registro Encontrado!" },
...
<script type="text/javascript">
    ...
    function editar(id, descricao, fornecedor, valor, data_vencimento, data_lancamento, data_pagamento, forma_pagamento, frequencia, obs, arquivo, multa, juros, desconto, taxa, subtotal) {
        $('#mensagem').text(''); $('#titulo_inserir').text('Editar Registro'); $('#id').val(id);
        $('#descricao-perfil').val(descricao); $('#fornecedor-perfil').val(fornecedor);
        $('#valor-conta').val(valor); $('#vencimento-conta').val(data_vencimento);
        $('#pagamento-conta').val(data_pagamento); $('#forma_pagamento').val(forma_pagamento);
        $('#frequencia').val(frequencia); $('#obs-perfil').val(obs);
        ...
        if (arquivo && arquivo !== 'sem-foto.png') { $('#target-arquivo').attr("src", "./images/pagar/" + arquivo); }
        else { $('#target-arquivo').attr("src", "./images/pagar/sem-foto.png"); }
    ...
    function mostrar(descricao, fornecedor, valor, data_vencimento, data_lancamento, data_pagamento, forma_pagamento, frequencia, obs, arquivo, multa, juros, desconto, taxa, subtotal, usuario_lanc, usuario_pgto) {
        $('#titulo_dados').text('Detalhes: ' + descricao); $('#descricao_dados-cli').text(descricao);
        ...
        $('#forma_pagamento_dados-cli').text(forma_pagamento); $('#frequencia_dados-cli').text(frequencia);
        $('#obs_dados-cli').text(obs || '-'); $('#multa_dados-cli').text(multa); $('#juros_dados-cli').text(juros);
        $('#desconto_dados-cli').text(desconto); $('#taxa_dados-cli').text(taxa); $('#subtotal_dados-cli').text(subtotal);
        ...
        } else { $('#target-arquivo-dados').attr('src', './images/pagar/sem-foto.png'); $('#link-arquivo-dados').hide(); }
        $('#usuario_lanc_dados-cli').text(usuario_lanc); $('#usuario_pgto_dados-cli').text(usuario_pgto);
    ...
    function limparCampos() {
        ...
        $('#arquivo-conta').val(''); $('#target-arquivo').attr('src', './images/pagar/sem-foto.png');
        $('#mensagem').text('').removeClass('text-danger');
    }

    function selecionar(id) {
        var ids = $('#ids').val();
        if ($('#seletor-'+id).is(":checked")) { $('#ids').val(ids + id + '-'); }
        else { $('#ids').val(ids.replace(id + '-', '')); }
        $('#btn-deletar').toggle($('#ids').val() !== "");
    }

    function deletarSel() { var ids = $('#ids').val().split("-"); for (var i=0; i<ids.length-1; i++) { if (ids[i]) excluir(ids[i]); } limparCampos(); }

    function excluir(id) {
        $.ajax({ url: 'paginas/'+pag+"/excluir.php", method: 'POST', data: {id:id}, dataType: "html",
    ...
    function baixar(id, valor, descricao, forma_pgto, data_vencimento) {
        $('#id-baixar').val(id); $('#descricao-baixar').text(descricao); $('#valor-baixar').val(valor);
        if (forma_pgto && forma_pgto !== 'undefined') { $('#saida-baixar').val(forma_pgto).change(); }
        $('#subtotal-baixar').val(valor); $('#valor-juros, #valor-desconto, #valor-multa, #valor-taxa').val('');
        $('#data-vencimento-baixar').val(data_vencimento);
        $('#modalBaixar').modal('show'); $('#mensagem-baixar').text('');
    }
</script>