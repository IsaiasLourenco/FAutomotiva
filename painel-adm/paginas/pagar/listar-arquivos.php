...
if (!$id_conta) {
    echo "<tr><td colspan='4' class='text-center text-muted py-3'>Nenhum arquivo</td></tr>";
    exit;
}
...
if (empty($arquivos)) {
    echo "<tr><td colspan='4' class='text-center text-muted py-3'>Nenhum arquivo anexado</td></tr>";
    exit;
}
...
foreach ($arquivos as $a) {
    ...
    // ✅ URLs CORRETAS para o módulo PAGAR
    $url_visualizar = "paginas/pagar/download-arquivo.php?id={$id_arq}&acao=visualizar";
    $url_baixar = "paginas/pagar/download-arquivo.php?id={$id_arq}&acao=baixar";
...
HTML;
    ...
    if ($pode_visualizar) {
        echo '<a href="' . $url_visualizar . '" target="_blank" class="btn btn-outline-primary" title="Visualizar">
                  <i class="fa fa-eye"></i>
              </a>';
    }

    // ✅ Botão Baixar (para todos)
    echo '<a href="' . $url_baixar . '" target="_blank" class="btn btn-outline-success" title="Baixar">
